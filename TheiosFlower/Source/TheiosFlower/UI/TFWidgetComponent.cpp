...
void UTFWidgetComponent::InitWidget()
{
	...
	UTFUserWidget* TFUserWidget = Cast<UTFUserWidget>(GetWidget()); //GetWidget()을 통해 현재 컴포넌트가 소유중인 위젯을 가져온다.
	if (TFUserWidget)
	{
		TFUserWidget->SetOwningActor(GetOwner()); //GetOwner()를 통해 자신이 소유중인 액터정보를 가져와서 넘겨준다.
